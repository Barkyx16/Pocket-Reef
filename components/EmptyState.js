...
export function EmptyState({ emoji = "🐠", title, subtitle }) {
  return (
    <View style={{ alignItems: "center", paddingVertical: 22, paddingHorizontal: 10 }}>
      ...
      {title ? <Text style={{ color: "#fff", fontSize: 16, fontWeight: "900", marginTop: 14, textAlign: "center" }}>{title}</Text> : null}
      {subtitle ? <Text style={{ color: theme.secondaryText, fontSize: 13, fontWeight: "600", marginTop: 6, textAlign: "center", lineHeight: 19, maxWidth: 300 }}>{subtitle}</Text> : null}
    </View>
  );
}
