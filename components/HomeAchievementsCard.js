  ...
  return (
    <Pressable onPress={() => { tapHaptic(); onPress && onPress(); }} accessibilityRole="button" accessibilityLabel={`Achievements: ${earned} of ${total} unlocked`}>
      <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
        <Text style={{ color: "#fff", fontSize: 18, fontFamily: "Inter_900Black", fontWeight: "900" }}>{earned} <Text style={{ color: theme.secondaryText, fontSize: 14 }}>/ {total} badges</Text></Text>
        <Text style={{ color: theme.accent, fontSize: 12, fontFamily: "Inter_900Black", fontWeight: "900" }}>View all ›</Text>
      </View>
        ...
        <View style={{ flexDirection: "row", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
          {next.map((a) => (
            <View key={a.id} style={{ flexDirection: "row", alignItems: "center", gap: 6, backgroundColor: "rgba(255,255,255,0.05)", borderWidth: 1, borderColor: theme.border, borderRadius: 999, paddingHorizontal: 10, paddingVertical: 6 }}>
              ...
              <Text style={{ color: theme.secondaryText, fontSize: 11, fontFamily: "Inter_800ExtraBold", fontWeight: "800" }}>{a.title}</Text>
            </View>
          ))}
        </View>
      ) : (
        <Text style={{ color: theme.accent, fontSize: 12, fontFamily: "Inter_800ExtraBold", fontWeight: "800", marginTop: 10 }}>Every badge earned — legendary reefer! 🏆</Text>
      )}
    </Pressable>
  );
}
