...
export function FleetCard({ tanks = [], activeTankId, reminderPrefs, onSwitch, now }) {
  ...
  return (
    <View>
      <Text style={{ color: "#fff", fontSize: 15, fontFamily: "Inter_900Black", fontWeight: "900" }}>{fleet.headline}</Text>
        ...
        {fleet.ranked.map((p, i) => {
          ...
          return (
            <Pressable
              ...
              style={({ pressed }) => [{ flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: on ? "rgba(56,225,198,0.10)" : theme.well, borderRadius: 14, borderWidth: 1, borderColor: on ? theme.accent : alertColor ? `${alertColor}44` : theme.border, padding: 11 }, pressed && { opacity: 0.8 }]}
            ...
            >
              <Text style={{ color: theme.secondaryText, fontSize: 12, fontFamily: "Inter_900Black", fontWeight: "900", width: 14 }}>{i + 1}</Text>
              <Text style={{ fontSize: 18 }}>{p.emoji}</Text>
              <View style={{ flex: 1 }}>
                <Text numberOfLines={1} style={{ color: "#fff", fontSize: 14, fontFamily: "Inter_900Black", fontWeight: "900" }}>{p.name}{on ? " · active" : ""}</Text>
                <Text style={{ color: alertColor || theme.secondaryText, fontSize: 11.5, fontFamily: "Inter_700Bold", fontWeight: "700", marginTop: 1 }}>
      ...
      {fleet.differences.length ? (
        <View style={{ marginTop: 14, backgroundColor: theme.well, borderRadius: 14, borderWidth: 1, borderColor: theme.border, padding: 12 }}>
          ...
          <View style={{ flexDirection: "row", marginTop: 10, marginBottom: 4 }}>
            <Text style={{ flex: 1 }} />
            <Text style={{ width: 62, textAlign: "right", color: theme.accent, fontSize: 10.5, fontFamily: "Inter_900Black", fontWeight: "900" }} numberOfLines={1}>{fleet.best.name}</Text>
            <Text style={{ width: 62, textAlign: "right", color: theme.secondaryText, fontSize: 10.5, fontFamily: "Inter_900Black", fontWeight: "900" }} numberOfLines={1}>{fleet.worst.name}</Text>
          </View>
          {fleet.differences.slice(0, 4).map((d) => (
            <View key={d.id} style={{ flexDirection: "row", alignItems: "center", paddingVertical: 4, borderTopWidth: 1, borderTopColor: theme.hairline }}>
              <Text style={{ flex: 1, color: theme.text, fontSize: 12, fontFamily: "Inter_800ExtraBold", fontWeight: "800" }}>{d.label}</Text>
              <Text style={{ width: 62, textAlign: "right", color: theme.accent, fontSize: 12.5, fontFamily: "Inter_900Black", fontWeight: "900" }}>{d.best}</Text>
              <Text style={{ width: 62, textAlign: "right", color: theme.secondaryText, fontSize: 12.5, fontFamily: "Inter_900Black", fontWeight: "900" }}>{d.worst}</Text>
            </View>
          ))}
          <Text style={{ color: theme.bodyText, fontSize: 11.5, fontFamily: "Inter_600SemiBold", fontWeight: "600", lineHeight: 16, marginTop: 8 }}>
      ...
      {fleet.needsAttention.length ? (
        <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginTop: 12 }}>
          ...
          <Text style={{ flex: 1, color: theme.warn, fontSize: 12, fontFamily: "Inter_800ExtraBold", fontWeight: "800" }}>
