    ...
    <View>
      ...
      {sources.length > 1 ? (
        <View style={{ gap: 6, marginBottom: 12 }}>
          {sources.map((s) => (
            <View key={s.id} style={{ flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: theme.well, borderRadius: 12, borderWidth: 1, borderColor: theme.border, paddingHorizontal: 11, paddingVertical: 8 }}>
          ...
          {mayDoubleCount ? (
            <Text style={{ color: theme.secondaryText, fontSize: 11, fontFamily: "Inter_700Bold", fontWeight: "700", lineHeight: 16 }}>
      ...
      <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginTop: 8 }}>
        ...
        {editingDate ? (
          <TextInput
            ...
            style={{ flex: 1, backgroundColor: theme.well, borderRadius: 10, paddingHorizontal: 10, paddingVertical: 6, color: theme.text, borderWidth: 1, borderColor: dateValid ? theme.accent : theme.danger, fontSize: 13, fontFamily: "Inter_800ExtraBold", fontWeight: "800" }}
        ...
        <View style={{ marginTop: 14, gap: 8 }}>
          {costs.slice(0, visible).map((c) => (
            <View key={c.id} style={{ flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: theme.well, borderRadius: 12, padding: 10, borderWidth: 1, borderColor: theme.border }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ color: theme.text, fontSize: 13, fontFamily: "Inter_800ExtraBold", fontWeight: "800" }} numberOfLines={1}>{c.label}</Text>
                {costDate(c) ? (
