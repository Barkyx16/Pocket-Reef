...
export function fmt(value, opts) {
  // Destructuring a default only covers undefined, not null, so `fmt(x, null)`
  // threw where `fmt(x)` was fine. Nothing calls it that way today; the guard
  // costs a line and removes the difference.
  const { sig = SIG, max = 2 } = opts || {};
