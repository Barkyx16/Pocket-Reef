...
// Writes any text to a shareable file in the cache. Same contract as
// writeBackupFile — {ok, uri, filename, bytes} or ok:false with a reason —
// because the water-log CSV needs exactly this and had been handing its whole
// contents to Share.share as a message body instead. That is the thing the top
// of this file was written about: Messages truncates it, Notes chokes, and the
// one thing nobody can do is save it and open it in a spreadsheet. It mattered
// less when a tank kept sixty water tests; the cap is now a thousand.
export async function writeTextFile(text, filename) {
  const body = String(text == null ? "" : text);
  if (Platform.OS === "web") return { ok: false, reason: "no-filesystem", text: body };

  const FileSystem = fs();
  if (!FileSystem || !FileSystem.Paths || !FileSystem.File || !FileSystem.Directory) {
    return { ok: false, reason: "no-filesystem", text: body };
  }
  try {
    ...
    file.create();
    file.write(body);
    return { ok: true, uri: file.uri, filename, bytes: byteLength(body), text: body };
  } catch (e) {
    return { ok: false, reason: "write-failed", text: body };
  }
}

// pocket-reef-water-log-2026-08-17.csv
export function waterLogFilename(now = new Date()) {
  const d = new Date(now);
  const stamp = Number.isNaN(d.getTime()) ? "export" : dayKey(d);
  return `pocket-reef-water-log-${stamp}.csv`;
}
