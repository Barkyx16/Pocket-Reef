...
export function recommendFor(p, waterTests = [], opts = {}) {
  ...
  const [lo, hi] = p.good;
  const outOfRange = current < lo || current > hi;
  // Math.abs made "two units past the limit" look identical to "two units of
  // room left", so a reading far outside the band read as having plenty of
  // headroom. Outside the band there is none.
  const distance = outOfRange ? 0 : Math.min(current - lo, hi - current);
  ...
  if (outOfRange) {
    // A reading already outside its safe band is tested until it is back
    // inside, whatever it is doing. This asked only "is it changing?", so a
    // tank sitting flat at 5 ppm ammonia — lethal, and not moving because
    // nothing had been done about it — was told every 30 days was plenty and
    // that testing weekly was too often. The one case where the answer has to
    // be "keep looking" was the case it answered "stop looking".
  ...
  return {
    ...
    outOfRange,
    reason: outOfRange
      ? `${round(current, 2)}${p.unit ? ` ${p.unit}` : ""} is outside the safe range — keep testing daily until it's back inside.`
      : !moving
