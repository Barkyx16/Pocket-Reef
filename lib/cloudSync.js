...
export function buildSnapshot(state) {
  const out = {};
  // Reached from a queued write that may run long after the state it was given,
  // so a missing object returns an empty snapshot rather than throwing inside
  // the sync queue where nothing is watching.
...
export async function fetchServerEntitlement(userId) {
  ...
  try {
    ...
    //
    // A timestamp that won't parse has to be treated the same way, and wasn't:
    // getTime() gave NaN, NaN > now is false, and the row came back as "not
    // entitled". The header of this function promises null when the answer
    // isn't known, and a garbled expiry is exactly that — the subscriber's
    // access should not turn on whether a webhook wrote a date we can read.
    let notExpired = true;
    if (data.expires_at) {
      const at = new Date(data.expires_at).getTime();
      if (Number.isNaN(at)) return null;   // unreadable, so unknown
      notExpired = at > Date.now();
    }
