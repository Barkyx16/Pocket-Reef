...
export function TabBarScrollProvider({ children }) {
  ...
  // Visibility is a ref plus a listener set, NOT context state.
  //
  // When `hidden` lived in the context value, every hide/show changed that
  // value's identity and re-rendered every consumer — including the whole app
  // shell and whichever 300-row list was on screen. Scrolling a long list
  // stuttered for no reason other than the nav bar changing its mind. Now only
  // the bar itself re-renders.
  ...
  const setHidden = useCallback((next) => {
    ...
    listeners.current.forEach((fn) => { try { fn(next); } catch (e) {} });
  }, []);

  const subscribeHidden = useCallback((fn) => {
    listeners.current.add(fn);
    return () => listeners.current.delete(fn);
  }, []);
  const getHidden = useCallback(() => isHidden.current, []);
  ...
  const value = useMemo(
    () => ({ onScroll, reveal, setActiveKey, getOffset, clearOffset, subscribeHidden, getHidden }),
    [onScroll, reveal, setActiveKey, getOffset, clearOffset, subscribeHidden, getHidden]
  );
...
// Subscribes ONLY the calling component to visibility changes, so scrolling a
// long list never re-renders the app shell around it.
export function useTabBarHidden() {
  const ctx = useContext(TabBarScrollContext);
  const [hidden, setHidden] = useState(() => (ctx ? ctx.getHidden() : false));
  useEffect(() => {
    if (!ctx) return;
    setHidden(ctx.getHidden());
    return ctx.subscribeHidden(setHidden);
  }, [ctx]);
  return hidden;
}
