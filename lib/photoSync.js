...
import { isPersisted } from "./photoStore";
...
// Is this URI a file this device actually holds? Only true for photos this
// build copied into its own documents folder, and only when the file is still
// there — a URI from another phone matches neither.
function localPhotoExists(uri) {
  if (!uri || !isPersisted(uri)) return false;
  try {
    const FileSystem = require("expo-file-system");
    if (!FileSystem || !FileSystem.File) return false;
    return Boolean(new FileSystem.File(uri).exists);
  } catch (e) {
    // No filesystem to check against: fall through to signing, which at least
...
// The entry id becomes a path segment in the photo bucket. Ids are generated
// here, but they also arrive from imported backups and synced profiles, where
// nothing has ever checked them — and a value carrying slashes or dots builds
// a key that is not the one the caller meant. Restricting it to the characters
// an id actually uses costs nothing and removes the question.
...
export async function uploadPhoto(userId, localUri, entryId) {
  ...
  try {
    ...
    const path = `${safeSegment(userId)}/${safeSegment(entryId) || Date.now()}.${ext}`;
...
export async function backupTankPhotos(userId, tanks) {
  ...
  const next = await Promise.all(
    tanks.map(async (tk) => {
      ...
      // Per-tank, not the shared counter: these run concurrently, so testing
      // the shared total here made a tank's result depend on whether some
      // OTHER tank happened to finish an upload first.
      let tankUploaded = 0;
      ...
      uploaded += tankUploaded;
      return tankUploaded ? { ...tk, journal: entries } : tk;
    })
  );
    ...
    tanks.map(async (tk) => {
      ...
      const entries = await Promise.all(
        journal.map(async (e) => {
          ...
          // A local file:// URI from ANOTHER device is useless here, but the
          // one that took the photo still has the file, and this ran on it too
          // — swapping a working local path for a signed URL made the device
          // holding the original depend on the network to see it, and those
          // URLs are deliberately short-lived, so it broke when they expired.
          // The header of this file says the local URI stays put; now it does.
          if (localPhotoExists(e.photo)) return e;
