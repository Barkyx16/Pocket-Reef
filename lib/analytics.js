...
//   * Nothing leaves the device. This is a local ring buffer the Profile screen
//     can summarize. If you later ship remote analytics, it must be opt-in and
//     disclosed — an aquarium journal is personal, and App Store privacy
//     labels have to match what the app actually does.
...
export async function track(event, reason) {
  try {
    ...
    buffer.push({
      e: event,
      r: reason && ALLOWED_REASONS.has(reason) ? reason : undefined,
      t: Date.now(),
    });
