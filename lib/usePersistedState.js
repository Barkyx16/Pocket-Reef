...
export function usePersistedState(key, initial, { validate } = {}) {
  ...
  useEffect(() => {
    ...
    return () => { alive = false; };
  // eslint-disable-next-line react-hooks/exhaustive-deps -- `validate` is usually an inline predicate, so a new function each render; adding it would re-read storage forever.
  }, [key]);
