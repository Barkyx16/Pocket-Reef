...
// Exported for tests. This decides which reminders exist, how they are worded
// and which tank each is about — the substance of the whole module — and it
// was unreachable from a test, which is most of why this file sat at 38%
// covered. Changing it could only be verified by reading the source back,
// which proves what was written and not what it produces.
export function buildReminders({ reminderPrefs = {}, tanks = [], tankName = "your tank", topAction = null, streakAtRisk = false, forecasts = [] }) {
  const out = [];
  ...
  const add = (key, days, title, body, to) => {
    ...
    //
    // `tankIds` says which tanks this reminder is actually about. Answering
    // "Done" from the lock screen used to record against whichever tank was
    // open in the app, which for a two-tank keeper meant the reef's water
    // change was logged on the quarantine tank — and the reef stayed overdue.
    out.push({ key, title, body, to, intervalDays: interval, hour: CARE_HOUR,
               category: CATEGORY.chore, action: key, tankIds: dueIds(key) });
  };

  // Which tanks a given reminder key is due for. Feeding has no per-tank
  // signal, so it claims every tank and the caller resolves the ambiguity.
  const dueIds = (key) => {
    const flag = key === "waterTest" ? "testDue" : key === "waterChange" ? "changeDue" : null;
    const list = flag ? tanks.filter((t) => t && t[flag]) : tanks;
...
// Exported for tests: date arithmetic that decides when a keeper is woken up.
export function nextOccurrence(days, hour) {
...
export function secondsUntilHour(hour) {
  const now = new Date();
    ...
    try {
      ...
      await Notifications.scheduleNotificationAsync({
        content: {
          title: r.title,
          body: r.body,
          data: { to: r.to, key: r.key, tool: r.tool || null, action: r.action || null, tankIds: r.tankIds || null },
          sound: "default",
...
export function readResponse(response) {
  ...
  if (action === ACTION.done) return { kind: "done", key: data.key || null, to: data.to || null, action: data.action || null, tankIds: Array.isArray(data.tankIds) ? data.tankIds : [] };
