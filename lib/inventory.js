...
export function forecastItem(item, tank = {}, opts = {}) {
  ...
  // newInventoryItem always sets a stock figure, so anything this app wrote has
  // one. A record from an imported backup or a profile synced from an older
  // build may not, and the two ways of not having it behaved differently:
  // `null <= 0` is true so it reported "Out of stock", while `undefined <= 0`
  // is false so it fell through and divided, giving a headline that read
  // "About NaN days left — null" beside a state of "ok".
  //
  // Absence is not zero. Zero means "you have none, buy some"; absence means
  // nobody has said, and claiming the shelf is empty about a record that never
  // carried the field is a different and wrong statement.
  const raw = item.stock;
  const onHand = raw === null || raw === undefined || raw === "" ? NaN : Number(raw);
  if (!Number.isFinite(onHand)) {
    return {
      ...base,
      state: "unknown",
      headline: "No amount recorded — set how much you have to get a run-out date",
    };
  }
  ...
  const daysLeft = Math.floor(onHand / rate.perDay);
