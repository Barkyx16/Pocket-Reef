...
// RevenueCat prefixes sandbox keys with "test_". Shipping with one is a silent
// revenue failure: the SDK configures, offerings come back empty, and the
// paywall reports "Plans aren't available right now — check your connection",
// which is a lie nobody can act on. Nothing crashes, nothing logs, and the only
// symptom is that no one ever subscribes.
//
// So it is asked out loud instead. The paywall says what is actually wrong, and
// a test ties this state to the note above so neither can be quietly removed
// while the other still applies.
export const usingTestKeys = () => String(IOS_API_KEY).startsWith("test_") || String(ANDROID_API_KEY).startsWith("test_");
