...
export function planDose({ key, current, ratedGallons, strengthPerUnit, targetOverride }) {
  ...
  // needed * volume / strength.
  //
  // `strength` is how much ONE ml raises the parameter in ONE gallon, which is
  // what the field asks for ("dKH raised per ml, per gallon"). One ml spread
  // through V gallons therefore raises it by strength/V, so reaching `needed`
  // takes needed * V / strength millilitres.
  //
  // This divided by volume instead of multiplying, which is wrong by V squared
  // — on a 100 gallon tank, 8,100 times too small. Checked against a real
  // label: Seachem Reef Carbonate is 5 ml per 20 gallons for 2.8 dKH, so
  // raising a 90 gallon tank by 1.5 dKH is about 12 ml. The app said 0.0015.
  //
  // The error direction was under-dosing, so nothing was ever harmed — but the
  // keeper doses what it says, nothing moves, and the tool is worse than
  // useless because it looks like it worked.
  const totalMl = Math.round((needed * volume / strength) * 100) / 100;
