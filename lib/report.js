...
import { formatVolume, localiseParam, tempToDisplay } from "./units";
...
// `unitLabel` is no longer taken from the caller. It used to be, and App passed
// "L" for a metric keeper without converting the number — so a 75 gallon tank
// was reported as "75 L". That is not a cosmetic slip: 75 gallons is 284 L, so
// the document a keeper hands to a shop or posts on a forum understated their
// tank by nearly four times. formatVolume does both halves, and has all along.
export function buildTankReport(tank, { now = new Date(), strengths = {} } = {}) {
  if (!tank) return "";
  ...
  const overview = [
    line("Tank", tank.name),
    line("Volume", formatVolume(tank.gallons)),
    line("Type", water),
  ...
  const waterBody = latest
    ? params
        .map((raw) => {
          const v0 = latest.values ? latest.values[raw.key] : null;
          if (v0 == null) return null;
          // Temperature is stored in °F and shown in whichever unit the keeper
          // reads. The report showed °F to everyone, so a metric keeper's own
          // report disagreed with every other screen in their app.
          const p = localiseParam(raw);
          const v = raw.key === "temp" ? tempToDisplay(v0) : v0;
  ...
  const wcBody = wc.count
    ? [
      ...
      wc.volume30 ? `Changed in 30 days: ${formatVolume(wc.volume30)}${wc.turnover30 != null ? ` (${wc.turnover30}% of volume)` : ""}` : null,
