...
export function livestockSpend(stock = [], stockMeta = {}, quantities = {}, losses = []) {
  ...
  const lost = losses.reduce((sum, l) => sum + (typeof l.price === "number" ? l.price * l.count : 0), 0);
...
export function mortalitySummary(losses = [], { days = 365, now = new Date() } = {}) {
  ...
  inWindow.forEach((l) => {
    ...
    byCause[c] = (byCause[c] || 0) + l.count;
  });

  const total = inWindow.reduce((n, l) => n + l.count, 0);
  ...
  return {
    ...
    repeatOffenders: Object.entries(
      inWindow.reduce((acc, l) => { acc[l.name] = (acc[l.name] || 0) + l.count; return acc; }, {})
