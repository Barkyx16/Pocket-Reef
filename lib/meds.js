...
export function planMedDose({ labelDose, labelPer, ratedGallons, waterChangePct = 0, unit = "ml" }) {
  ...
  return {
    ...
    // "on the box" was accurate while both numbers were gallons. Converted for a
    // metric keeper it would describe a box that says 284 L when it says 75
    // gallons, so the phrasing now refers to the tank's rating rather than to
    // what is printed on the bottle.
    volumeNote: `Calculated on about ${formatVolume(actual)} of actual water, not the ${formatVolume(rated)} the tank is rated for — rock, sand and the gap below the rim take up the difference.`,
    topUpNote: pct
